# Authored SVG arena rule model

This bounded model covers the product rule transitions shared by `Domain/ArenaContent.fs`
and `Domain/ArenaRules.fs`: collection, goal completion, hazard damage, terminal guards,
and restart. Geometry, rendering, input devices, transport, and persistence stay outside
the model. Each user-visible rule operation is one atomic transition.

```quint arena-rules.qnt +=
module SvgArenaRules {
  type State = {
    health: int,
    score: int,
    collected: bool,
  ...
  pure def collectNext(s) = { ...s, collected: true, score: s.score + 100, lastAction: "collect" }
  ...
  pure def restartNext(s) = { ...initialState, round: s.round + 1, lastAction: "restart" }
  ...
  val scoreShape = state.score == 0 or state.score == 100
  val collectedScore = not(state.collected) or state.score == 100
  ...
  val witnessCollect = state.collected and state.score == 100
  val witnessWin = state.outcome == "won"
  val witnessLoss = state.outcome == "lost" and state.health == 0
  val witnessRestart = state.round > 1 and state.outcome == "playing" and state.health == 3
...
The actions correspond to `ArenaContent.interact`, `ArenaRules.advance`, and
`ArenaRules.applyIntent Restart`. The invariants correspond to the same source and
are exercised in .NET and Fable by `Protocol.Tests/cross-runtime/Program.fs`.
